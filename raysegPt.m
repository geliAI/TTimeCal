function [ segPoint ] = raysegPt( velMod,depthMod,delta,depthEvent,...
...
% evt_loc = [-delta/2,-depthEvent];
% sta_loc = [delta/2,0];
evt_loc = [-delta,-depthEvent];
sta_loc = [0,0];
if isRefract == 0  % direct wave
    raysegNum = evtL+1;%Num of seg Pts.
   ...
   cumdistN = cumsum(distN(end:-1:1));
    cumeleN = cumsum([deplayerTop,thickness(evtL-1:-1:1)]);%Elevation change
